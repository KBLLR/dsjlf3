...
export const App = () => (
  <Canvas orthographic camera={{ position: [0, 0, 100], zoom: 100 }}>
    <FrostedGlass>
      <OrthographicCamera makeDefault position={[0, 0, 100]} zoom={200} />
      <mesh position={[2, 0, 0]}>
        <sphereGeometry args={[4.25, 64, 64]} />
        <meshStandardMaterial color="#333" envMapIntensity={0.3} metalness={1} roughness={0.2} />
      </mesh>
      <Ambience />
    </FrostedGlass>
  </Canvas>
...
function FrostedGlass({ children }) {
  ...
  return (
    <mesh scale={[width, height, 1]}>
      <planeGeometry />
      <MeshTransmissionMaterial samples={10} thickness={0.1} chromaticAberration={0} anisotropy={1} roughness={1}>
